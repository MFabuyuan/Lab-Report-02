/*
LCD with Arduino to work as ohmmeter with display of the Ω symbol
...
This program uses LCD custom character to display the ohm (Ω) symbol on the LCD screen.
*/
...
void setup() {
  ...
  lcd.createChar(0, customChar);     //create a custom character to display the ohm (Ω) symbol
...
void loop() {
  ...
  float x = (val/1023.0)*5.0;      //calculate the voltage value from the analog reading   
  float Ia = (5.0-x/0.999);        //calculate the current flowing through the resistor (Ra)
  float Ib = Ia;                   //assign the value of Ia to Ib
  float Vb = x;                    //assign the value of Vb to x
  float Rb = (Vb/Ib);              //calculate the value of the unknown resistor(Rb)
  ...
  lcd.write((byte)0);        //display Ω symbol on the LCD screen


//display the bit, voltae, current(Ia), and unknown resistor value(Rb) on the Serial monitor
